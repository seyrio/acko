...
class MedicalRAGRetriever:
    ...
    def load_documents_from_redis(self, redis_client, key_pattern="transcription_stream"):
        """Load documents from Redis with proper type handling"""
        try:
            # Check if it's a specific stream name or pattern
            if key_pattern == "transcription_stream":
                # Handle Redis stream
                return self._load_from_redis_stream(redis_client, key_pattern)
            else:
                # Handle pattern matching for different key types
                return self._load_from_redis_keys(redis_client, key_pattern)
                
        except Exception as e:
            logger.error(f"Failed to load documents from Redis: {e}")

    def _load_from_redis_stream(self, redis_client, stream_name):
        """Load documents from Redis stream"""
        try:
            ...
            if documents:
                self.add_documents(documents)
                logger.info(f"Loaded {len(documents)} documents from Redis stream: {stream_name}")
            else:
                logger.warning(f"No messages found in Redis stream: {stream_name}")
                
        except Exception as e:
            logger.error(f"Error reading from Redis stream {stream_name}: {e}")

    def _load_from_redis_keys(self, redis_client, key_pattern):
        """Load documents from Redis keys with type detection"""
        try:
            ...
            for key in keys:
                ...
                if key_type == 'hash':
                    doc_data = redis_client.hgetall(key_str)
                    if doc_data:
                        # Convert bytes to strings if needed
                        if isinstance(doc_data, dict):
                            doc_data = {k.decode() if isinstance(k, bytes) else k: 
                                      v.decode() if isinstance(v, bytes) else v 
                    ...
                    if content:
                        ...
                        documents.append({
                            'id': key_str,
                            'content': content,
                            'metadata': {'source': 'redis_string'}
                        })
                        
                elif key_type == 'stream':
                    # Handle individual stream
                    messages = redis_client.xrange(key_str, count=1000)
                    for msg_id, fields in messages:
                        if isinstance(fields, dict):
                            fields = {k.decode() if isinstance(k, bytes) else k: 
                                    v.decode() if isinstance(v, bytes) else v 
                    ...
                    logger.warning(f"Unsupported Redis key type '{key_type}' for key: {key_str}")
            
            if documents:
                self.add_documents(documents)
                logger.info(f"Loaded {len(documents)} documents from Redis with pattern: {key_pattern}")
            else:
                logger.warning(f"No documents found in Redis with pattern: {key_pattern}")
                
        except Exception as e:
            logger.error(f"Error loading from Redis keys with pattern {key_pattern}: {e}")
...
class RAGFollowUpGenerator:
    ...
    def read_conversation_from_redis(self, stream_name='transcription_stream', count=100) -> str:
        """Read conversation from Redis stream with error handling"""
        try:
            # Check if the key exists and is a stream
            if not self.redis_client.exists(stream_name):
                logger.warning(f"Stream {stream_name} does not exist")
                return ""
            
            key_type = self.redis_client.type(stream_name)
            if key_type != 'stream':
                logger.error(f"Key {stream_name} is not a stream (type: {key_type})")
                return ""
